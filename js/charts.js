function configureChart(ChartJsProvider) {
  ChartJsProvider.setOptions({
    chartColors: [ // http://www.mulinblog.com/a-color-palette-optimized-for-data-visualization/
      '#FAA43A', // persija
      '#5DA5DA', // persib
      '#60BD68', // persebaya
      '#4D4D4D', // perseden
...
function makeChartData() {
  ...
  var chart = {
    bar: {
      order: {
        labels: cities,
        data: cities.map(city => data[city]['-1']['orders']['all']),
        options: {
          title: {
      ...
      amount: {
        labels: cities,
        data: cities.map(city => data[city]['-1']['amount']['all']),
        options: {
          title: {
            text: 'Total tarif (Rp)',
          },
      ...
      distance: {
        labels: cities,
        data: cities.map(city => data[city]['-1']['distance']['all']),
        options: {
          title: {
            text: 'Total jarak (km)',
          },
    ...
    line: {
      order: {
        ...
        data: cities.map(city => {
          return hoursKey.map(hour => data[city]['-1']['orders'][hour]);
        }),
        datasetOverride: Array.apply(null, cities).map(function() { return { fill: false } }),
        options: {
          legend: {
            display: true,
          },
      ...
      amount: {
        ...
        series: cities,
        data: cities.map(city => {
          return hoursKey.map(hour => data[city]['-1']['amount'][hour]);
        }),
        datasetOverride: Array.apply(null, cities).map(function() { return { fill: false } }),
        options: {
          title: {
      ...
      distance: {
        ...
        series: cities,
        data: cities.map(city => {
          return hoursKey.map(hour => data[city]['-1']['distance'][hour]);
        }),
        datasetOverride: Array.apply(null, cities).map(function() { return { fill: false } }),
        options: {
          title: {
